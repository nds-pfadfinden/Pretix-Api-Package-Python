...
class QuestionsApi(BaseAPI):
    def create(self, event_slug: str, data: dict):
        r = self.client.session.post(
            f'{self.client.config["events_url"]}{event_slug}/questions/', json=data
        )
        return self._check_response(r)

    def get(self, event_slug: str) -> list[dict]:
        questions = self._handle_pagination(
            self.client.config["events_url"] + event_slug + "/questions/"
        )
        return questions

    def patch(self, event_slug: str, question_id: int, data: dict) -> dict:
        r = self.client.session.patch(
            f'{self.client.config["events_url"]}{event_slug}/questions/{question_id}/',
            json=data,
        )
        return self._check_response(r)
